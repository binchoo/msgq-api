...
/*a notation attached to a function that returns 0 for success, -1 for failure*/
#define __linux_return 
/*a notation attached to a function that returns true or false */
#define __bool_return 
...
inline int __bool_return chk_noref(struct msgq* queue);
...
int __bool_return peek_msg(struct msgq* queue, struct msgbuf* msg, unsigned int bytes);
...
int __linux_return add_msg(struct msgq* queue, struct msgbuf* msg, unsigned int bytes) {
	int retval = -1;
	int limit;
	printk("current volume of queue = %d\n", get_volume(queue));
...
int sub_msg(struct msgq* queue, struct msgbuf* msg, unsigned int bytes) {
	...
	if (queue->ref > 0 && queue->len > 0) {
		if (0 == find_node_by_type(&node, queue, type)) {
			limit = __min(sizeof(struct msgbuf), bytes);
			//strncpy(msg->text, node->msg->text, limit - (int)((void*)&msg->text - (void*)msg));
